    ...
    <form ref={formRef} className="authForm ethiopianSignup" onSubmit={submit} noValidate>
      ...
      <fieldset hidden={step !== 1}>
        ...
        <div className="authGrid">
          <label>
            Full legal name
            <input name="fullName" required autoComplete="name" maxLength={80} placeholder="Mahir Aman" />
          </label>
          <label>
            Ethiopian mobile number
            <input name="phone" required autoComplete="tel" inputMode="tel" placeholder="0911 234 567" />
          </label>
      ...
      <fieldset hidden={step !== 2}>
        ...
        <div className="identityNotice">
          ...
          <span>Mezgeb stores the ID type and only the final four characters. The complete document number is not stored until an approved verification service is connected.</span>
        </div>
        <div className="authGrid">
          <label>
            Ethiopian ID type
            <select name="idType" value={idType} onChange={(event) => setIdType(event.target.value as EthiopianIdType)}>
              {ETHIOPIAN_ID_TYPES.map((type) => <option key={type.value} value={type.value}>{type.label}</option>)}
            </select>
          </label>
          <label>
          ...
          <label>
            Confirm password
            <input type="password" name="passwordConfirmation" minLength={8} required autoComplete="new-password" />
          </label>
        </div>
        ...
        <label className="authConsent">
          ...
          <span>I understand that my ID type and final four characters are retained for account-security purposes, while the full number is discarded.</span>
        </label>
        <div className="signupActions">
          <button className="button textButton" type="button" onClick={() => setStep(1)}>← Back</button>
