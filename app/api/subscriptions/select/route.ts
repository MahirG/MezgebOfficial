...
export async function POST(request: Request) {
  ...
  if (!planCodePattern.test(planCode) || !supportedCycles.has(billingCycle)) {
    return NextResponse.json(
      { error: 'Choose a valid Mezgeb plan and billing interval.' },
      { status: 400 }
    );
  }
  ...
  if (!plan)
    return NextResponse.json(
      { error: 'That Mezgeb plan is not currently available.' },
      { status: 400 }
    );
    ...
    return NextResponse.json(
      {
        error: duplicateSubscription
          ? 'A Mezgeb subscription already exists for this account.'
          : error.message
      },
